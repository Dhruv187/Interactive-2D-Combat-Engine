...
export class MultiBattleScene {
  constructor(changeScene = null, stage = null) {
    ...
    // Multiplayer: throttle playerMove emissions (send max 30 times/sec)
    this._lastEmitTime = 0;
    this._emitInterval = 1000 / 30;

    // Use provided stage or create from gameState
    ...
    // Wire up multiplayer socket if in multi mode
    if (gameState.mode === "multi") {
      this.setupMultiplayerSocket();
    }
  }
  ...
  setupMultiplayerSocket() {
    const localId = gameState.playerId; // 0 or 1
    const remoteId = 1 - localId; // the opponent

    console.log(`[BATTLE] 🎮 Multiplayer mode — I am player ${localId}`);
    ...
    this.onOpponentMove = ({ playerId, moveData }) => {
      if (playerId !== remoteId) return;
      const remoteFighter = this.fighters[remoteId];
      if (!remoteFighter) return;

      // Apply the received state snapshot
      remoteFighter.position.x = moveData.position.x;
      remoteFighter.position.y = moveData.position.y;
    ...
    this.onOpponentDisconnect = () => {
      console.warn("[BATTLE] ⚠️ Opponent disconnected!");
      // Freeze the game and reload after a short delay
      this.battleEnded = true;
      this._disconnectMessage = true;
      setTimeout(() => {
  ...
  // Emit local fighter state to server (throttled to 30/sec)
  emitLocalMove(time) {
    if (gameState.mode !== "multi") return;
    if (time.previous - this._lastEmitTime < this._emitInterval) return;
    this._lastEmitTime = time.previous;

    const localId = gameState.playerId;
    const f = this.fighters[localId];
    if (!f) return;
  ...
  createStage() {
    // Use the selected stage from gameState (stored as lowercase by HostScene)
    const selectedStage = (gameState.selectedStage || "ken").toLowerCase();
  ...
  update(time, context) {
    ...
    // Multiplayer: send local player state to server this frame
    this.emitLocalMove(time);
  }
  ...
  draw(context) {
    ...
    if (this._disconnectMessage) {
      // Opponent disconnected — show overlay
      ...
      context.fillText(
        "Opponent disconnected. Returning to menu...",
